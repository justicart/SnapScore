...
export const SettingsView: React.FC<SettingsViewProps> = ({ settings, onSave, onCancel, isClient, onLeave }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-canvas relative">
      <div className="flex items-center p-4 bg-surface shadow-sm">
        <button onClick={onCancel} className="p-2 -ml-2 text-ink-muted hover:text-white">
          <IconChevronLeft className="w-6 h-6" />
        </button>
        <h2 className="text-lg font-bold ml-2 text-white">Game rules</h2>
      </div>
      ...
      <div className="p-6 space-y-10 overflow-y-auto flex-1">
        ...
        <div className="space-y-3">
            <label className="block text-sm font-medium text-primary-soft uppercase tracking-widest">Game preset</label>
             <button
                onClick={() => setIsPresetSheetOpen(true)}
                className="w-full flex items-center justify-between bg-surface border border-surface-highlight rounded-xl p-4 text-left hover:border-primary transition-colors group"
            >
                <div className="flex items-center gap-3">
                    <div className="p-2 bg-surface-highlight rounded-lg group-hover:bg-primary/20 group-hover:text-primary-soft transition-colors text-ink-muted">
                    ...
                    <div>
                        ...
                        <span className="block text-xs text-ink-muted">{currentPreset.desc}</span>
                    </div>
                </div>
                <IconChevronDown className="w-5 h-5 text-ink-subtle group-hover:text-primary-soft" />
            </button>
        </div>
        ...
        {formData.preset === 'standard' ? (
            <div className="space-y-6 animate-in fade-in duration-500">
                ...
                <div className="space-y-2">
                    <label className="block text-sm font-medium text-primary-soft uppercase tracking-wider">Winning condition</label>
                ...
                {/* Number Cards */}
                <div className="space-y-2">
                    <label className="block text-sm font-medium text-primary-soft uppercase tracking-wider">Number cards (2-10)</label>
                    <div className="flex gap-2 p-1 bg-surface rounded-lg">
                        <button
                        type="button"
                    ...
                    {formData.numberCardBehavior === 'fixed' && (
                        <div className="mt-2 bg-surface p-3 rounded-lg border border-surface-highlight">
                        <input 
                            type="number" 
                ...
                {/* Face Cards */}
                <div className="space-y-2">
                    <label className="block text-sm font-medium text-primary-soft uppercase tracking-wider">Face cards (K, Q, J)</label>
                    <div className="flex gap-2 p-1 bg-surface rounded-lg">
                        <button
                        type="button"
                    ...
                    {formData.faceCardBehavior === 'fixed' && (
                        <div className="mt-2 bg-surface p-4 rounded-lg border border-surface-highlight flex items-center gap-4">
                            <span className="text-2xl font-black text-primary-soft w-10">{formData.fixedFaceValue || 10}</span>
                            <input 
                                type="range" min="0" max="50" step="1"
                                value={formData.fixedFaceValue || 10}
                                onChange={(e) => handleChange('fixedFaceValue', parseInt(e.target.value))}
                                className="flex-1 accent-primary h-2 bg-surface-highlight rounded-lg appearance-none cursor-pointer"
                            />
                        </div>
                    )}
                </div>
                ...
                {/* Aces */}
                <div className="space-y-2">
                    <label className="block text-sm font-medium text-primary-soft uppercase tracking-wider">Aces</label>
                    <div className="bg-surface p-4 rounded-lg border border-surface-highlight flex items-center gap-4">
                        <span className="text-2xl font-black text-primary-soft w-10">{formData.aceValue}</span>
                        <input 
                            type="range" min="0" max="50" step="1"
                ...
                {/* Jokers */}
                <div className="space-y-2">
                    <label className="block text-sm font-medium text-primary-soft uppercase tracking-wider">Jokers</label>
                    <div className="bg-surface p-4 rounded-lg border border-surface-highlight flex items-center gap-4">
                        <span className="text-2xl font-black text-primary-soft w-10">{formData.jokerValue}</span>
                        <input 
            ...
            <div className="bg-primary/5 border border-primary/20 p-4 rounded-xl animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="flex items-center gap-2 text-primary-soft mb-2">
      ...
      <div className="p-4 border-t border-surface bg-canvas">
      ...
      {isPresetSheetOpen && (
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-end justify-center sm:items-center p-4 sm:p-0" onClick={() => setIsPresetSheetOpen(false)}>
            <div 
                className="bg-surface-dark w-full max-w-md rounded-2xl border border-surface p-6 space-y-4 animate-in slide-in-from-bottom duration-200 shadow-2xl mb-4 sm:mb-0"
                onClick={e => e.stopPropagation()}
            >
                <div className="flex justify-between items-center mb-2">
                    <h3 className="text-xl font-bold text-white">Select preset</h3>
                    <button onClick={() => setIsPresetSheetOpen(false)} className="p-2 -mr-2 text-ink-muted hover:text-white rounded-full">
                    ...
                    {PRESETS.map(p => (
                        <button
                            ...
                            className={`w-full flex items-center justify-between p-4 rounded-xl border-2 text-left transition-all ${
                                ...
                                : 'bg-surface/50 border-surface-highlight hover:bg-surface'
                            }`}
                        >
                            <div className="flex items-center gap-3">
                                 <div className={`p-2 rounded-lg ${formData.preset === p.id ? 'bg-primary/20 text-primary-soft' : 'bg-surface-highlight text-ink-muted'}`}>
                                    {p.id === 'standard' ? <IconSettings className="w-5 h-5" /> : <IconStar className="w-5 h-5" />}
                                 </div>
                                 <div>
                                    <span className={`block font-bold ${formData.preset === p.id ? 'text-primary-soft' : 'text-white'}`}>{p.name}</span>
                                    <span className="text-xs text-ink-muted">{p.desc}</span>
                                </div>
                            </div>
                            {formData.preset === p.id && <div className="bg-primary rounded-full p-1"><IconCheck className="w-3 h-3 text-white" /></div>}
                        </button>
                    ))}
                </div>
            </div>
